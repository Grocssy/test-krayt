...
var offsetBlockSelector = '#header';
...
function scrollToTarget(event) {
	...
	if ($target.length > 0) {
		...
		var targetPos = $target.offset().top - $(offsetBlockSelector).outerHeight();
		var deltaY = Math.abs($(window).scrollTop() - targetPos);
		var time = Math.round(deltaY / 2);
...
var linkSelector = '.js-scroll-to-target';
...
var _jsScrollToTarget = __webpack_require__(1);

var _jsScrollToTarget2 = _interopRequireDefault(_jsScrollToTarget);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var offsetBlockSelector = '#header';
var menuSelector = ".page-nav-menu";
var linkSelector = menuSelector + ' a';
...
function inViewport($block) {
	var fromTop = $(window).scrollTop() + $(offsetBlockSelector).innerHeight();
	var blockTop = $block.offset().top;
	var blockH = $block.innerHeight();

	var scrolledToBlockTop = fromTop >= blockTop;
	var scrolledToBlockBottom = fromTop > Math.floor(blockTop + blockH);

	return scrolledToBlockTop && !scrolledToBlockBottom;
}

function setClickListener() {
	$(linkSelector).on('click', _jsScrollToTarget2.default);
}
...
var linkSelector = '#toggle-menu-link';

function toggleMenu(event) {
	event.preventDefault();

	var $link = $(linkSelector);
	var menuSelector = $link.attr('href');
...
var sectionSelector = '#portfolio';
var gallerySelector = "#portfolio-gallery";
var sliderSelector = gallerySelector + " .slick-slide";
var innerSelector = ".slick-slide__inner";
...
function stylizeSlides(slick, activeIndex) {
	...
	var maxLevel = Math.floor(slideCount / 2);

	$(sliderSelector).each(function () {
		var $slide = $(this);
		var slideIndex = $slide.data('slick-index');
		var slideLevel = getSlideLevel(slideIndex, activeIndex, slideCount);

		var slideZIndex = maxLevel - slideLevel;
		var $inner = $(innerSelector, $slide);
		var slideHPadding = $inner.innerWidth() * sizeMultiplier * slideLevel + 'px';
...
function getSlideLevel(currentIndex, activeIndex, slideCount) {
	var levelInArray = Math.abs(currentIndex - activeIndex);
	var levelCrossArray = slideCount - currentIndex + Math.abs(activeIndex);
...
var sectionSelector = '#reviews';
var gallerySelector = '#reviews-gallery';
